...
void read_input_file(std::string filepath, std::string& hex_p, std::string& hex_g, std::string& hex_a, std::string& hex_b){
...
int main(int argc, char* argv[]){
    ...
    std::string hex_p, hex_g, hex_a, hex_b;
    read_input_file(input_file, hex_p, hex_g, hex_a, hex_b);
    BigInt p(hex_p);
    BigInt g(hex_g);
    BigInt a(hex_a);
    BigInt b(hex_b);
    BigInt k = BigInt::powModBigInt(g, a, p);
    k = BigInt::powModBigInt(k, b, p);
    std::string hex_k = k.toHexString();
    write_output_file(output_file, hex_k);
    return 1;
}