...
void read_input_file(std::string filepath, std::string& hex_p, std::string& hex_a, std::string& hex_b, std::string& hex_xP, std::string& hex_yP, std::string& hex_xQ, std::string& hex_yQ){
    std::ifstream f(filepath);
    std::getline(f, hex_p, ' ');
    std::getline(f, hex_a, ' ');
    std::getline(f, hex_b);
    std::getline(f, hex_xP, ' ');
    std::getline(f, hex_yP);
    std::getline(f, hex_xQ, ' ');
    std::getline(f, hex_yQ);
    f.close();
}

void write_output_file(std::string filepath, std::string hex_xR, std::string hex_yR){
...
std::tuple<bool, BigInt>inverse_mod_BigInt(BigInt a, BigInt mod){
    ...
    return std::make_tuple(1, x0);
}

int main(int argc, char* argv[]){
    std::string input_file = argv[1];
    std::string output_file = argv[2];
    std::string hex_p, hex_a, hex_b, hex_xP, hex_yP, hex_xQ, hex_yQ;
    read_input_file(input_file, hex_p, hex_a, hex_b, hex_xP, hex_yP, hex_xQ, hex_yQ);
    BigInt p(hex_p);
    BigInt a(hex_a);
    BigInt xP(hex_xP);
    BigInt yP(hex_yP);
    ...
    BigInt yQ(hex_yQ);
    BigInt num, den;
    if (hex_xP == hex_xQ && hex_yP == hex_yQ){ // P = Q => lamda = (3xP^2 + a)/2yP
        num = BigInt::multiplyBigInt(BigInt(std::vector<unsigned long>(1, 3), 1), BigInt::multiplyBigInt(xP, xP));
        num = BigInt::subtractBigInt(num, BigInt(a.getNumber(), !a.getSign()));
        den = BigInt::multiplyBigInt(BigInt(std::vector<unsigned long>(1, 2), 1), yP);
    }
    else { // P != Q => lamda = (yQ - yP)/(xQ - xP)
        num = BigInt::subtractBigInt(yQ, yP);
        den = BigInt::subtractBigInt(xQ, xP);
        num.setSign(!(num.getSign()^den.getSign()));
        den.setSign(1);
    }
    bool is_coprime;
    BigInt den_inverse;
    std::tie(is_coprime, den_inverse) = inverse_mod_BigInt(den, p);
    if (!is_coprime){
        std::cout << "gcd is not equal to 1";
        return 1;
    }
    BigInt lamda = BigInt::modBigInt(BigInt::multiplyBigInt(num, den_inverse), p);
    // xR = lamda^2 - xP - xQ (mod p)  
    BigInt xR = BigInt::subtractBigInt(BigInt::subtractBigInt(BigInt::multiplyBigInt(lamda, lamda), xP), xQ);
    xR = BigInt::modBigInt(xR, p);
    // yR = lamda(xP - xR) - yP (mod p)
    BigInt yR = BigInt::subtractBigInt(BigInt::multiplyBigInt(lamda, BigInt::subtractBigInt(xP, xR)), yP);
    yR = BigInt::modBigInt(yR, p);
    std::string hex_xR = xR.toHexString();
    std::string hex_yR = yR.toHexString();
    write_output_file(output_file, hex_xR, hex_yR);
    return 1;
}