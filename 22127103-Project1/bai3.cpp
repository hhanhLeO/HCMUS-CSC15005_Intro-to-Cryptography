...
void read_input_file(std::string filepath, int& x, int& y, std::string& hex_n, std::string& hex_e, std::vector<std::string>& hex_m, std::vector<std::string>& hex_c){
    ...
    for (int i = 0;i < x;i++){
        std::getline(f, line);
        hex_m.push_back(line);
    }
    for (int i = 0;i < y;i++){
        std::getline(f, line);
        hex_c.push_back(line);
    }
    f.close();
}
...
int find_cipher(std::string c, std::vector<std::string> list){
    int len = list.size();
    for (int i = 0;i < len;i++){
...
int main(int argc, char* argv[]){
    ...
    int x, y;
    std::string hex_n, hex_e;
    std::vector<std::string> hex_m, hex_c;
    read_input_file(input_file, x, y, hex_n, hex_e, hex_m, hex_c);
    BigInt n(hex_n);
    BigInt e(hex_e);
    BigInt temp;
    std::string c_i;
    std::string res = "";
    for (int i = 0;i < x;i++){
        temp = BigInt::powModBigInt(BigInt(hex_m[i]), e, n);
        c_i = temp.toHexString();
        int index = find_cipher(c_i, hex_c);
